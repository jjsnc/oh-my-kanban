...
test("添加新卡片并移到进行中", async ({ page }) => {
  ...
  // 2. 用户添加新卡片
  // 2.1 点击“添加新卡片”
  const todoListColumn = page.locator('[data-testid="todo-column"]', {
    hasText: "待处理",
  });
  ...
  await newCardTitleText.waitFor({ state: "visible" }); // 等待输入框可见
  await expect(newCardTitleText).toBeFocused();
  await newCardTitleText.fill("测试E2E任务-1");
  ...
  const ongoingListColumn = page.locator('[data-testid="todo-column"]', {
    hasText: "进行中",
  });
